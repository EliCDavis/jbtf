...
type jbtfSerializableStruct struct {
	data int32
}

func (pss *jbtfSerializableStruct) Deserialize(in io.Reader) (err error) {
	data := make([]byte, 4)
...
func (pss jbtfSerializableStruct) Serialize(out io.Writer) (err error) {
	bytes := make([]byte, 4)
...
type embeddedBinaryStruct struct {
	Basic        basicStruct
	Serializable *jbtfSerializableStruct
}

type multipleEmbeddedBinaryStruct struct {
	Basic         basicStruct
	SerializableA *jbtfSerializableStruct
	SerializableB *jbtfSerializableStruct
}
...
func (tc typedTestCase[T]) Run(t *testing.T) {
	out, err := jbtf.Marshal(tc.input)
	assert.NoError(t, err)
	assert.Equal(t, tc.want, string(out))

	v, err := jbtf.Unmarshal[T](out)
	assert.NoError(t, err)
...
func TestMarshal(t *testing.T) {
	...
	"data": {
		...
		"embedded binary": typedTestCase[embeddedBinaryStruct]{
			input: embeddedBinaryStruct{
				...
				Serializable: &jbtfSerializableStruct{data: 12345},
			},
			want: `{
	"buffers": [
		{
	...
	"data": {
		...
		"multiple embedded binary": typedTestCase[multipleEmbeddedBinaryStruct]{
			input: multipleEmbeddedBinaryStruct{
				Basic:         basicStruct{A: 123, B: true, C: "yee haw"},
				SerializableA: &jbtfSerializableStruct{data: 12345},
				SerializableB: &jbtfSerializableStruct{data: 67890},
			},
			want: `{
	"buffers": [
		{
	...
	"data": {
		...
		"nil embedded binary": typedTestCase[embeddedBinaryStruct]{
			input: embeddedBinaryStruct{
				Basic: basicStruct{A: 123, B: true, C: "yee haw"},
			},
			want: `{
